...
>
    ...
    <div class="space-y-2">
        <label class="block text-sm font-semibold text-white">Image Feed</label>

        <div class="relative w-full aspect-[4/5] bg-slate-800 border-2 border-dashed border-gray-500 rounded-xl overflow-hidden flex items-center justify-center">
            <input
                type="file"
                name="media"
                x-ref="fileInput"
            ...
            <template x-if="previews.length > 0">
                <div class="relative w-full h-full">
                    <img :src="previews[0]" class="absolute inset-0 w-full h-full object-cover hover:grayscale-50" />
                    <button
                        type="button"
                        @click.prevent="removeImage(0)" class="absolute top-2 right-2 bg-red-600/80 hover:bg-red-700 text-white p-1 rounded-full">
    ...
    <div class="space-y-3">
        ...
        {{-- <input type="hidden" name="hashtag" :value="JSON.stringify(allTags)"> --}}
            <div class="flex flex-wrap gap-2">
                <template x-for="(tag, index) in optionalTags" :key="tag">
                    <span class="inline-flex items-center px-3 py-1.5 text-sm font-normal bg-gradient-to-b from-cyan-500 to-sky-600 hover:from-sky-600 hover:to-blue-600 text-white rounded-2xl">
